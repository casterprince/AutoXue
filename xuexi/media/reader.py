    ...
    def _star_share_comment(self, title):
        ...
        if pos_comment:
            ...
            if pos_publish2 == pos_publish:
                ...
                try:
                    x = max(self.ad.wmsize)
                    # 这里做了一条一次函数拟合，也不知道能不能适应所有情况
                    offset = 0.0203*x+0.7595  #cfg.getint('offset', f'{x}')
                except Exception as e:
                    logger.debug(f'不好意思，没有找对偏移量数据,请使用推荐分辨率设置')
                    raise AttributeError(f'没有针对分辨率{self.ad.wmsize}的偏移量推荐分辨率1024x576、1280x720')                    
